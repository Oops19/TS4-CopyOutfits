...
from copy_outfits.struct.actions_id import O19COActionId
from copy_outfits.struct.age import O19COAge
...
class OutfitTools(CommonTerrainInteraction):
    ...
    def on_started(self, interaction_sim: Sim, interaction_target: Any) -> bool:
        ...
        if outfit_index == -1 or outfit_index == _outfit_index:
            ...
            update_outfit = False
        outfit_name = O19COOutfitStore.outfit_categories.get(outfit_category)
        ...
        # action: 1 Copy || 2 Paste || 4 O19COActionId || 8 Clear || 16 Print
        # action_id: 0..100
        # outfit_index: 0..5 | -1 Default
        # outfit_category: 0..12 | -1 Default
        # clipboard_index: 0..4
        if action == O19COAction.PRINT:
            log.debug(f"Outfits: {O19COOutfitStore.outfits}")
            ...
            if action_id == O19COActionId.ALL:
                ...
                if age_data:
                    ...
                    O19COOutfitStore.outfits.update({age: age_data})
            ...
            valid_parts_ids = O19COOutfitStore.parts[O19COActionId.CLOTH]
    ...
    def get_parts(age: int, clipboard_index: int = -1) -> dict:
        parts = dict()
        debug_message = ''
        age_data = O19COOutfitStore.outfits.get(age, dict())
        if age_data:
